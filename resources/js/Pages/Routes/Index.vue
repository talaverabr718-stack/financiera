...
<template>
    <AppLayout title="Rutas de cobranza" eyebrow="Operación en campo" description="Organiza la jornada y registra cada visita.">
        <template #header-actions>
            <div class="flex gap-2">
                <input type="date" :value="String(date).slice(0, 10)" class="control m-0" @change="router.get(endpoints.index, { date: $event.target.value })">
                <a :href="endpoints.create" class="btn-primary">Nueva ruta</a>
            </div>
        </template>
                    ...
                    <div class="divide-y">
                        <div v-for="stop in selectedRoute.stops" :key="stop.id" class="p-4">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="badge" :class="stop.status === 'visited' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'">{{ stop.status }}</span>
