...
<template>
    <div class="grid grid-cols-2 gap-2 lg:grid-cols-4">
        ...
        <article class="col-span-2 rounded-xl border border-indigo-100 bg-gradient-to-r from-indigo-50 to-cyan-50 p-3 lg:col-span-4">
            <div class="flex flex-wrap items-center justify-between gap-2 text-[10px] font-semibold"><span class="text-indigo-700">Capital {{ ((Number(result.principal) / Number(result.total_payment)) * 100).toFixed(1) }}%</span><span v-if="result.method === 'monthly_flat'" class="rounded-full bg-white px-2 py-1 text-emerald-700">Porcentaje total {{ Number(result.total_rate).toFixed(2) }}%</span><span class="text-amber-700">Interés {{ ((Number(result.total_interest) / Number(result.total_payment)) * 100).toFixed(1) }}%</span></div>
