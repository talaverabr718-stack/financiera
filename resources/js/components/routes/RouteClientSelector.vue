...
<template>
    ...
    <BaseModal :open="Boolean(detail)" title="Información del cliente" size="max-w-md" @close="detail=null"><div v-if="detail" class="space-y-3 text-xs"><div><span class="label">Cliente</span><strong class="block">{{ detail.full_name }} · {{ detail.code }}</strong></div><div><span class="label">Dirección</span><p>{{ detail.address || 'Sin dirección registrada' }}</p></div><div class="grid grid-cols-2 gap-3"><div><span class="label">Municipio</span><p>{{ detail.municipality || '—' }}</p></div><div><span class="label">Barrio</span><p>{{ detail.neighborhood || '—' }}</p></div></div><div class="rounded-xl p-3" :class="isMapped(detail) ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'"><strong class="block">{{ coordinates(detail) }}</strong><p class="mt-1">{{ isMapped(detail) ? 'Punto disponible para planificar la visita.' : 'Este cliente no aparecerá como punto en el mapa.' }}</p><a v-if="isMapped(detail)" :href="mapsUrl(detail)" target="_blank" rel="noopener" class="mt-2 inline-flex rounded-lg bg-white px-3 py-2 text-[10px] font-bold text-emerald-700 shadow-sm">Abrir ubicación exacta ↗</a></div></div></BaseModal>
</template>

<style scoped>
.stat{display:flex;align-items:center;gap:.4rem;border-radius:.65rem;background:#f8fafc;padding:.45rem .6rem;color:#64748b}.stat strong{font-size:.85rem;color:#334155}.stat span{font-size:.52rem;font-weight:700;text-transform:uppercase}.stat-green{background:#ecfdf5}.stat-green strong{color:#059669}.stat-amber{background:#fffbeb}.stat-amber strong{color:#d97706}.filter-pill{border-radius:.5rem;padding:.35rem .6rem;font-size:.55rem;font-weight:800;color:#94a3b8}.filter-active{background:#eef2ff;color:#4f46e5}.status{flex:none;border-radius:9999px;padding:.25rem .5rem;font-size:.52rem;font-weight:800}.status-mapped{background:#ecfdf5;color:#059669}.status-unmapped{background:#fffbeb;color:#d97706}.status-locked{background:#f1f5f9;color:#64748b}.label{display:block;margin-bottom:.2rem;font-size:.55rem;font-weight:800;text-transform:uppercase;color:#94a3b8}
</style>
