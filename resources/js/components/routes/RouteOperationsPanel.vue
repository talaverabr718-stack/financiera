...
const props = defineProps({ routesCount: Number, totalStops: Number, initialVisited: Number, initialPending: Number, hasSelectedRoute: Boolean, stops: Array, csrf: String, endpointTemplate: String, dashboardTarget: String, completeRedirect: String });
...
const confirmVisit = async () => {
    ...
    try {
        ...
        selected.value.visited_at_label = data.stop.visited_at_label;
        selected.value = null;
        if (data.route_status === 'completed' && props.completeRedirect) {
            window.location.assign(props.completeRedirect);
        }
