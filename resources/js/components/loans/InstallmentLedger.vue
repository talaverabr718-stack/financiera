        ...
        <div class="table-wrap max-h-[min(28rem,62vh)] overflow-auto">
            <table class="w-full text-left">
                <thead class="text-[10px] uppercase tracking-wide text-slate-400"><tr>
                    ...
                    <th class="table-cell">#</th><th class="table-cell">Vence</th><th class="table-cell text-right">Cuota original</th><th class="table-cell text-right">Pagado</th><th class="table-cell text-right">Saldo vencido</th><th class="table-cell text-right">Mora</th><th class="table-cell text-right">Total a pagar</th><th class="table-cell">Situación</th><th class="table-cell">Historial</th>
                </tr></thead>
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="row in rows" :key="`${row.loan_id}-${row.number}`" :class="row.settlement === 'overdue' ? 'bg-rose-50/50' : row.settlement === 'on_time' ? 'bg-emerald-50/30' : ''">
                        ...
                        <td class="table-cell text-right tabular-nums">{{ money(row.installment_amount ?? row.amount_due) }}</td>
                        <td class="table-cell text-right tabular-nums">{{ money(row.amount_paid) }}</td>
                        <td class="table-cell text-right tabular-nums" :class="row.is_overdue ? 'font-semibold text-rose-700' : 'text-slate-400'">{{ row.is_overdue ? money(row.overdue_balance) : '—' }}</td>
                        ...
                        <td class="table-cell text-right font-semibold tabular-nums">{{ money(row.total_to_pay ?? row.outstanding_amount) }}</td>
                    ...
                    <tr v-if="!rows.length"><td :colspan="showLoan ? 10 : 9" class="p-8 text-center text-xs text-slate-400">{{ empty }}</td></tr>
                </tbody>
            </table>
        </div>
    </section>
