...
class SettingsController extends Controller
{
    ...
    public function modules() { return view('settings.modules', ['modules'=>SystemModule::orderBy('sort_order')->get()]); }
    ...
    public function updatePermissions(UpdatePermissionSettingsRequest $request)
    {
        DB::transaction(function () use ($request): void {
            ...
            foreach(User::get() as $user) foreach(SystemModule::get() as $module) {
    ...
    public function appearance() { return view('settings.appearance',['settings'=>$this->settings->group('appearance')]); }
    ...
    public function brand() { return view('settings.brand',['settings'=>$this->settings->group('brand')]); }
    public function updateBrand(UpdateBrandSettingsRequest $request)
    {
        $current=$this->settings->group('brand'); $values=$request->safe()->except(['logo','remove_logo']);
        if ($request->hasFile('logo')) { $values['logo_path']=$request->file('logo')->store('system-branding'); if (filled($current['logo_path']??null)) Storage::delete($current['logo_path']); }
        elseif ($request->boolean('remove_logo')) { if (filled($current['logo_path']??null)) Storage::delete($current['logo_path']); $values['logo_path']=null; }
        $this->settings->save('brand',$values,auth()->id());
        return back()->with('success','Nombre y logotipo del sistema actualizados.');
    }
    public function logo()
    {
        $path=$this->settings->group('brand')['logo_path']??null; abort_unless($path && Storage::exists($path),404);
        return Storage::response($path,null,['Cache-Control'=>'no-store, no-cache, must-revalidate','Pragma'=>'no-cache','Expires'=>'0']);
    }

    public function general() { return view('settings.general', ['settings' => $this->settings->group('general')]); }
    ...
    public function sequences() { return view('settings.sequences', ['sequences' => DB::table('document_sequences')->orderBy('key')->get()]); }
